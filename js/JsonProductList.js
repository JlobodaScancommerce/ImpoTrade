		...
		function parseJSON(jsondata,append){
			...
			if(!append){
				if($('#frontPageProductsBdy.jsonProducts').length > 0 || $('#frontPageProductsMostSoldBdy.jsonProducts').length > 0 || $('#basketProductsBdy.jsonProducts').length > 0){
					...
					divList = $("<ul class='products small-block-grid-1 medium-block-grid-2 large-block-grid-4'></ul>");
				}
				$(selector).empty();
			...
			$.each(jsondata.data.items, function(key, val) {
				...
				if(val.showAddToBasket = "true") {
					if(val.isBuyable = "true") {
						basketDiv = $("<div></div>");
						infoDiv=$("<a></a>");
						infoDiv.attr("href",val.URLPathAndQuery);
			...
			/*	if($('.searchResultsProductsOuterBdy').length > 0){
				...
				var productExtIdText="Varenr.: ";
				productExtIdDiv = $('<div></div>');
				productExtIdDiv.addClass('productExtId');
				productExtIdLink = $('<a></a>');
				productExtIdLink.attr("href", val.URLPathAndQuery);
				productExtIdLinkText = productExtIdText + " " + val.id;
				productExtIdLink.append(productExtIdLinkText);
				productExtIdDiv.append(productExtIdLink);
				productInnerDiv.append(productExtIdDiv);
				...
				if($('#favoriteProductsJsonBdy').length > 0){
					...
					productInnerDiv.append(infoDiv);
